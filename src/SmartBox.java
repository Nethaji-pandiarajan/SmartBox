...
public class SmartBox extends Application {
	...
	public static void main(String[] args) {
		...
		System.out.println("VERSION_DATED - 11/07/2022");
	...
	public static void deliveryMaterial(double sWidth, double sHeight) {
		...
		scan.setOnKeyReleased(eventAc -> {
			...
			if (scan.getText().length() <= 30) {
				if (eventAc.getCode() == KeyCode.ENTER) {
					...
					deliverySONumberCodeScan(soNumber, sp.getSceneWidth(), sp.getSceneHeight());
				}
		...
		scanButton.setOnAction(new EventHandler<ActionEvent>() {
			public void handle(ActionEvent e) {
				...
				SerialPort sp = SerialPort.getCommPort(ApiModule.getPropertyValue("PORT_CONF")); // device name TODO:
				...
				if (sp.openPort()) {
					System.out.println("Ardino Port Opened");
					byte[] b = "barcd".getBytes();
					// System.out.println("Barcode readed");

					sp.writeBytes(b, 20);

					System.out.println("barcd Sent");

					SerialPort sp2 = SerialPort.getCommPort(ApiModule.getPropertyValue("SCANNER_PORT")); // device name

					sp2.setComPortParameters(9600, 8, 1, 0); // default connection settings for Arduino
					sp2.setComPortTimeouts(SerialPort.TIMEOUT_WRITE_BLOCKING, 0, 0); // block until bytes can be written

					sp2.setFlowControl(SerialPort.FLOW_CONTROL_RTS_ENABLED);
					...
					if (sp2.openPort()) {
						...
						executor.submit(new Runnable() {
							...
							public void run() {
								...
								Scanner ardout = new Scanner(sp2.getInputStream());
						...
						System.out.println("Scanner Port is not opened");
						scan.setText("");
					...
					System.out.println("Ardino Port not Opened");
					scan.setText("");
				}
			}
		});
	...
	public static void deliverySONumberCodeScan(String soNumber, double sWidth, double sHeight) {
	...
	public static void deliverySOID(double sWidth, double sHeight) {
		...
		scan.setOnKeyReleased(eventAc -> {
			if (eventAc.getCode() == KeyCode.ENTER) {
				...
				deliverySOIDScanEnterCode(deliverySoidNumber, sp.getSceneWidth(), sp.getSceneHeight());
			}
		});
	...
	public static void deliverySOIDScanEnterCode(String deliverySoidNumber, double sWidth, double sHeight) {
		...
		scan.setOnKeyReleased(eventAc -> {
			if (scan.getText().length() <= 30) {
				if (eventAc.getCode() == KeyCode.ENTER) {
					...
					deliveryProductScanEnterCode(deliverySoidNumber, productCode, deliveryscannedSOIDtext,
							sp.getSceneWidth(), sp.getSceneHeight());
	...
	public static void deliveryProductScanEnterCode(String deliverySoidNumber, String productCode,
			Label deliveryscannedSOIDtext, double sWidth, double sHeight) {
		...
		if (isProductCodeValid) {
			...
			Validations.isValidConfirmMessage(deliverySoidNumber, lockname, productCode, qty, scannedSOIDint,
					totalSOIDint);
			gotoSetLayout();
		...
		scan.setOnKeyReleased(eventAc -> {
			if (scan.getText().length() <= 30) {
				if (eventAc.getCode() == KeyCode.ENTER) {
					...
					fulfillmentDCNumberCodeScan(dcNumber, sp.getSceneWidth(), sp.getSceneHeight());
				}
	...
	public static void fulfillmentDCNumberCodeScan(String dcNumber, double sWidth, double sHeight) {
	...
	public static void fulfillmentSOID(double sWidth, double sHeight) {
		...
		scan.setOnKeyReleased(eventAc -> {
			if (eventAc.getCode() == KeyCode.ENTER) {
				...
				fulfillmentSOIDScanEnterCode(fulfillmentSoidNumber, sp.getSceneWidth(), sp.getSceneHeight());
			}
		});
	...
	public static void fulfillmentSOIDScanEnterCode(String deliverySoidNumber, double sWidth, double sHeight) {
		...
		scan.setOnKeyReleased(eventAc -> {
			if (scan.getText().length() <= 30) {
				if (eventAc.getCode() == KeyCode.ENTER) {
					...
					fulfillmentProductscanEnterCode(fulSoidNumber, productCode, fulfillmentscannedSOIDtext,
							sp.getSceneWidth(), sp.getSceneHeight());
	...
	public static void fulfillmentProductscanEnterCode(String fulSoidNumber, String productCode,
			Label fulfillmentscannedSOIDtext, double sWidth, double sHeight) {
		...
		if (isProductCodeValid) {
			...
			if (lockerOpenTime1) {
				...
				fulfillmentscannedSOIDtext.setText("Scanned SOID: " + String.valueOf(fulfillmentscannedSOIDint));

				try {
					ApiModule.setpropertyValue("FULFILLMENT_SCANNED_SOID", String.valueOf(fulfillmentscannedSOIDint));
				} catch (Exception xx) {
					xx.printStackTrace();
				}
			...
			Validations.isValidConfirmMessage2(fulSoidNumber, lockname, productCode, qty, fulfillmentscannedSOIDint,
					fulFillmenttotalSOIDint);
			gotoSetLayout();
